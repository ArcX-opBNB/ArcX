...
public class PushCoinRechargeAdapter extends BaseQuickAdapter<RechargeResponse.GoldListDTO, MyViewHolder> {

    public PushCoinRechargeAdapter() {
        super(0);
    }
    ...
    protected MyViewHolder onCreateDefViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(AppUtil.getSize(66), AppUtil.getSize(69));
        layoutParams.topMargin = AppUtil.getSize(7);
        layoutParams.leftMargin = AppUtil.getSize(5);
        cardView.setLayoutParams(layoutParams);
        ConstraintLayout rootView = new ConstraintBuilder(66, 69).build(parent.getContext());
        cardView.addView(rootView);
        MyImageView myImageView = MyImageView.create(rootView, new ConstraintBuilder(40, 40).topCenterH());
        ...
        MyTextView BtnPay = MyTextView.create(rootView, new ConstraintBuilder(52, 15).bottom().bottomMargin(6).centerH());
